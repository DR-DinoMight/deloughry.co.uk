...
export default function LastPlayedBlock() {
  ...
  const items = `${data?.name} - ${data?.artist}`;
  const icon = (
  ...
  return (
    <StatCard name={title} icon={icon} className="border-spotify">
      <div className="flex items-center">
        {data?.albumArt &&<img src={data?.albumArt[2]?.url} alt={data?.name} className="h-8 w-8"/>}
        <p className="ml-2">
          <span className="text-l font-bold">{data?.name}</span> <br/>
